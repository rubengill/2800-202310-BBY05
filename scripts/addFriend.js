  ...
  userCard.innerHTML = `
    <div class="card rtl mx-auto text-center mb-3" style="max-width: 25rem;">
      ...
      <div class="card-body">
        ...
        <p class="card-text status">${userData.status}</p>
...
function processUserDocument(doc, emailHtml, userId) {
  ...
  userCard.querySelector('.status').textContent = userData.status ? 'Status: ' + userData.status : '';
  ...
  streakRef.get().then(function (snapshot) {
    if (snapshot.exists) {
      ...
      // Set up the "Add Friend" button with the friend data, current user ID, and streak data
      setupAddFriendButton(addFriendButton, friendData, userId, streakData, status);
    }
